...
def create_plots(
    ...
    if df.empty:
        print("Warning: No preference relations found. Skipping plot generation.")
        # Create a simple text file explaining the situation
        info_file = output_dir / "no_results_info.txt"
        with open(info_file, "w") as f:
            f.write(
                "No preference relations were found with the given epsilon and delta parameters.\n"
            )
            f.write(f"Parameters used:\n")
            f.write(f"  - max_set_size: {max_set_size}\n")
            f.write(f"  - top_features: {top_features}\n")
            f.write(f"  - n_rules: {n_rules}\n")
            ...
            f.write(
                "\nConsider adjusting epsilon (correlation threshold) or delta (importance difference) parameters.\n"
            )
        return
    ...
    for idx, epsilon in enumerate(epsilons):
        ...
        ax.set_xlabel("d", fontsize=14, labelpad=10)
        ax.set_ylabel("Average Dimensions", fontsize=14, labelpad=10)
        ax.set_title(f"e = {epsilon:.2f}", fontsize=16, pad=15)
